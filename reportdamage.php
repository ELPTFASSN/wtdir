    ...
    <div class="rptrow">
		...
		if(isset($_GET['filter'])){
			if($stmt->prepare("Select Concat(MonthName(`DCDate`), ' ',DayOfMonth(DCDate),', ', Year(DCDate)) as DCDate,DCDocNum
								From damagecontrol
								Inner Join inventorycontrol On damagecontrol.unInventoryControl = inventorycontrol.unInventoryControl
    ...
    <div class="rptrow">
		...
		if(isset($_GET['filter'])){
			if($stmt->prepare("Select Distinct PIName,
								(
									Select Sum(DDQuantity) From damagedata 
									Inner Join damagecontrol On damagedata.unDamageControl = damagecontrol.unDamageControl
									Inner Join inventorycontrol On damagecontrol.unInventoryControl = inventorycontrol.unInventoryControl
									Where unBranch = ? and (ICDate Between ? and ?) and unProductItem = src.unProductItem and damagecontrol.Status = 1 and damagedata.Status = 1
								) as `Quantity`,
								(
									Select PUOMName From productitem 	
								...
								Inner Join damagecontrol On src.unDamageControl = damagecontrol.unDamageControl
								Inner Join inventorycontrol On damagecontrol.unInventoryControl = inventorycontrol.unInventoryControl
								Where unBranch = ? and (ICDate Between ? and ?)
