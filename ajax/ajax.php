...
case 'loadbranchinfo':
		...
		if($stmt->prepare("SELECT unArea,unTemplateItemControl,unTemplateProductionBatch,BName,BDescription,BSAPCode,BType,BQuota,BQuotaInterval,BQuotaPointAmount 
							FROM branch  
							WHERE `Status`=1 And unBranch=?")){
			$stmt->bind_param('i',$_POST['bid']);
			$stmt->execute();
			$stmt->bind_result($unAreaBr,$unTemplateItemControlBr,$unTemplateProductionBatchBr,$BName,$BDescription,$BSAPCode,$BType,$BQuota,$BQuotaInterval,$BQuotaPointAmount);
			$stmt->fetch();
			$stmt->close();
		}
	?>		
           ...
           <?php
            ...
            <div class="popupitem">
                ...
                <?php
                    $mysql= new MySQLi($_SESSION['server'],$_SESSION['username'],$_SESSION['password'],$_SESSION['database']);
                    $stmt=$mysql->stmt_init();
                    if($stmt->prepare("Select templateproductionbatch.`unTemplateItemControl`,
						TICName,
						`unTemplateProductionBatch`,
						TPBName 
						From templateproductionbatch 
						INNER JOIN templateitemcontrol ON  
						templateitemcontrol.`unTemplateItemControl` = templateproductionbatch.`unTemplateItemControl` 
						Where templateproductionbatch.`Status`=1 Order by `TPBName`")){
                        $stmt->execute();
                        $stmt->bind_result($unTemplateItemControl,$TICName,$unTemplateProductionBatch,$TPBName);
                        while($stmt->fetch()){
							if($_SESSION['BusinessUnit']=="Waffletime Inc.,"){
				?>
                    <option value="<?php echo $unTemplateProductionBatch; ?>" <?php echo ($unTemplateProductionBatch==$unTemplateProductionBatchBr)?'Selected':''; ?>><?php echo $TPBName; ?></option>
                <?php
							}else{
                ?>
                    <option value="<?php echo $unTemplateProductionBatch; ?>" <?php echo ($unTemplateProductionBatch==$unTemplateProductionBatchBr)?'Selected':''; ?>><?php echo $TPBName; ?>[ Template: <?php echo $TICName; ?>]</option>
                <?php
							}
						}
						$stmt->close();
...
case 'loaddiscounttypeinfo':
	...
	?>		
        ...
        <form method="post" action="discounttype.php">
            ...
            <div class="popupitem">
                <div class="popupitemlabel">Vat Exempt</div><input autocomplete="off" name="txtvatexempt" type="text" style="width:195px; text-align:right;" value="<?php echo $DTVatExempt; ?>" placeholder="0.0000">
            </div>
...
<?php
		...
		if($stmt2->prepare("Select templateproductionbatch.unTemplateProductionBatch, templateproductioncontrol.unTemplateProductionControl, 
							...
							Where templateproductioncontrol.unTemplateProductionControl=?  AND templateproductionbatch.unTemplateItemControl=?
							And templateproductiondata.`Status`=1")){
			$stmt2->bind_param("ii", $_POST['bid'],$_POST['untic']);
			$stmt2->execute();
...
?>
            <div class="listviewitem" id="lvitem-<?php echo $i; ?>">
                ...
                <div class="listviewsubitem" style="width: 80px;"><input type="input" name="txtprocesstype-<?php echo $i; ?>" readonly style="border: none; background-color: transparent; width: inherit; text-align: right;" value="<?php //echo ($TPDProcessType==0)?'Sales':'Production';
				if($TPDProcessType==0){echo 'Sales';}else if($TPDProcessType==1){echo 'Production';}else if($TPDProcessType==2){echo 'Shared Usage';}else if($TPDProcessType==3){echo 'End Product';}?>"></div>
...
?>
	<div id="popme" class="popuptitle" align="center">[ <?php echo $PIName; ?> ] Production Data</div>
		<form method="post" action="productiontemplate.php?&id=<?php echo $_POST['bid']; ?>&type=<?php echo $_POST['type']; ?>">                                     
