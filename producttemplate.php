...
<?php
	...
	$mysqli = new MySQLi($_SESSION['server'],$_SESSION['username'],$_SESSION['password'],$_SESSION['database']);
		...
		if($stmt = $mysqli->prepare("Select unProductItem,PIName, producttype.unProductType, producttype.PTName, productitem.unProductGroup, productgroup.PGName  
			...
			while($stmt->fetch())
			{
				//echo ($unProductItem.$PIName.$unProductType.$PTName.$unProductGroup.$PGName);
	...
	if(isset($_POST['labeltransferitem']))
	{
		...
		if($stmt2 = $mysqli2->prepare("Select idTemplateItemData, unTemplateItemControl, templateitemdata.unProductItem, 
									...
									Inner Join producttype
									On producttype.unProductType = productgroup.unProductType
									WHERE templateitemdata.Status=1 AND unTemplateItemControl=? 
									Order By producttype.unProductType ASC, productgroup.PGPriority ASC, TIDPriority ASC")){
...
<div id="productitemcontainer"> <img src="img/icon/productitemtext.png" alt="PRODUCT ITEMS" />
  ...
  <form name="form1" action="producttemplate.php?id=<?php echo $_GET['id']; ?>" method="post">
    <div style="background-color:#FFF">
      ...
      <div class="listview" style="height:500px; overflow:auto;">
      	<?php 
		$row=0;
		...
		?>
        	<div class="row" id="rowproducttemplate" style="height:24px;">
        		<div class="listviewitem" style="background-color:#<?php echo ($row%2)?'FFF':'EEE'; ?>" id="lvitem-<?php echo $a; ?>" onClick="chkitemtoggle(<?php echo $a; ?>)">          
...
<form action="producttemplate.php?id=<?php echo $_GET['id']; ?>" method="post">
    <div id="toolbar">
          <input type="button" class="toolbarbutton" title="Save" name="btnSave" onClick="location.reload()" style="background-image:url(img/icon/save.png);" value="">
	...
	<div class="listview" id="lvproducttemplate">
      <div class="column" id="colproducttemplate" align="center">
        ...
        <div class="columnheader" style="width:250px;text-align:right;">Cost</div>
        <div class="columnheader" style="width:250px;text-align:right;">Price</div>
        <div class="columnheader" style="width:250px;text-align:right;">Priority</div>
      </div>
    ...
    <?php
        ...
        ?>
            <div class="listviewitem maintitem" style="background-color:#<?php echo ($c%2)?'EEE':'FFF'; ?>;" id="lvitem-<?php echo $c; ?>">
                ...
                <input type="text" value="<?php echo $clsMainPI->TIDCost; ?>" onKeyPress="return isNumberKey(event, 0)" onBlur="savefieldvalue('TIDCost','textcost-<?php echo $c;?>','<?php echo $clsMainPI->unTemplateItemData; ?>')" id="textcost-<?php echo $c; ?>" class="producttemplatesubitem" style="width:250px;text-align:right;">
                ...
                <input type="text" value="<?php echo $clsMainPI->TIDPrice; ?>" onKeyPress="return isNumberKey(event, 0)" onBlur="savefieldvalue('TIDPrice','textprice-<?php echo $c;?>','<?php /*echo '2';*/ echo $clsMainPI->unTemplateItemData; ?>')" id="textprice-<?php echo $c; ?>" class="producttemplatesubitem" style="width:250px;text-align:right;">
				</div>							
				...
				<input type="text" value="<?php echo $clsMainPI->TIDPriority; ?>" onKeyPress="return isNumberKey(event, 1)" onBlur="savefieldvalue('TIDPriority','textpriority-<?php echo $c;?>','<?php echo $clsMainPI->unTemplateItemData; ?>')" id="textpriority-<?php echo $c;?>" class="producttemplatesubitem" style="width:250px;text-align:right;">
				</div>
            </div>
      <?php
            }		
