    ...
    <div class="rptrow">
        ...
        if($stmt->prepare($query)){
            ...
            while ($stmt->fetch()){
				...
				// This is the hard-coded solution for [ TOTAL FOR WAFFLE(waffle + fom), TOTAL FOR BEVERAGES, GRAND TOTAL(TOTAL WATER + TOTAL BEVERAGES) ]
				// other option is to add another column at product group table for toggling of beverages and waffles

				if($PGName == 'Savory Fillings' || $PGName == 'Sweet Fillings' || $PGName == 'Fillings of the Month')
				{
					$TotalWaffleQty += $IDSoldQuantity;
				}
				elseif($PGName == 'Beverage' || $PGName == 'Other Drinks To Go' || $PGName == 'Ice Tea')
				{
					$TotalBeverageQty += $IDSoldQuantity;
				}
		...
		$row = array('TOTAL',$totalPract,'',$TotalAmount,$TotalPercent, $TotalWaffleQty, $TotalBeverageQty);
		$export[]=$row;
        ?>
		...
		<div class="rptlistviewitem" style="background-color:#FFF; margin-right: 5%; padding-top: 1%">
           <span style = "float:right; width: 45%">
			  <div class="rptlistviewsubitem" style="width: 35%;font-weight:bold" >| &nbsp; Breakdown of Total:</div>
			   <div class="rptlistviewsubitem" style="margin-right: 5%" >Waffles: <?php echo round($TotalWaffleQty) ?></div>
			   <div class="rptlistviewsubitem" style="margin-right: 5%" >Beverages: <?php echo round($TotalBeverageQty) ?></div>
			   <div class="rptlistviewsubitem" style="" >Grand Total: <?php echo round($TotalWaffleQty + $TotalBeverageQty) ?></div>
			</span>
        </div>
